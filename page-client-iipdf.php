...
if ( ! defined( 'ABSPATH' ) ) {
	die( 'Abs Not Authorised' );
} 
$client = new Client(do_client_security());
$invoice_item = new InvoiceItem($_GET['id']);
if ($client->id != $invoice_item->client_id) {
    die( 'Client Not Authorised' );
} 
...
$pdf = new FPDF();
$pdf->SetTitle($invoice_item->ii_title . ' ' . return_human_date($invoice_item->date));
$pdf->AddPage();
